...
REFRESH_NOTIFIED_KEY = "refresh_notified_v11"
VERIFICATION_EMOJIS = ["🍎", "🚗", "⭐", "⚽", "🐱"]
...
def app_action_callback(call):
    ...
    if action == "unlock":
        ...
        if cat_key == "freefire" and "proxy" in fname.lower():
            ...
            sent = send_local_apk(user_id, CARROM_FILENAME, f"📥 <b>Carrom Aim AI APK File:</b>")
            bot.send_message(user_id, f"🔑 <b>Key yahan se generate karein:</b> {FREE_KEY_BOT}", parse_mode="HTML")
        else:
            sent = send_local_apk(user_id, fname, f"📥 <b>{app_info['name']} APK File:</b>")
            if not sent:
                bot.send_message(user_id, f"⚠️ File server par nahi mili (`{fname}`). GitHub par upload karna na bhulein!")
        ...
        if cat_key == "carrom":
            send_local_apk(user_id, CARROM_FILENAME, f"📥 <b>Carrom Aim AI APK File:</b>")
            bot.send_message(user_id, f"🔑 <b>Key yahan se generate karein:</b> {FREE_KEY_BOT}", parse_mode="HTML")
        else:
            sent = send_local_apk(user_id, fname, f"📥 <b>{app_info['name']} APK File:</b>")
            if not sent:
                bot.send_message(user_id, f"⚠️ File server par nahi mili (`{fname}`). GitHub par upload karein.")
